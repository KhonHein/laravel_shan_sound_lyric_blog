        ...
        @else
            ...
            <a href="{{ route('user#postDetails',$p->id) }}" class="myElement col-sm-6 col-md-4 col-lg-3 border border-left-0  rounded-2 m-2 justify-content-center text-center rightContentChildCiv hoverOver">
                ...
                @else
                    ...
                    @endif
                @endif
                <p class="text-black fw-bold shanniFont fs-4">{{ $p->title }}</p>
                <audio src="{{ asset('storage/'.$p->sound) }}" controls class="my-2 ms-0 myAudio" type="audio/mpeg" style="width:250px; height:30px;"></audio>
            </a>
            @endforeach
        @endif
@endsection
